...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".fpl-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FPL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FPL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mbulling83/fpl-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.fpl-pp-cli/feedback.jsonl.
When ` + "`FPL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FPL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  fpl-pp-cli feedback list
  fpl-pp-cli feedback list --limit 5
  fpl-pp-cli feedback list --json`,
