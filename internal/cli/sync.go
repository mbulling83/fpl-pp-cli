...
Exit codes & warnings:
  ...
  fpl-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  fpl-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  fpl-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  fpl-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  fpl-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/fpl-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "bootstrap_static":
		return db.UpsertBootstrapStatic(data)
	case "entry":
		return db.UpsertEntry(data)
	case "entry_event":
	...
	case "fixtures":
		return db.UpsertFixtures(data)
	case "leagues_classic_standings":
		return db.UpsertLeaguesClassicStandings(data)
	case "leagues_h2h_standings":
		return db.UpsertLeaguesH2hStandings(data)
	default:
